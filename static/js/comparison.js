...
class ComparisonManager {
    ...
    async loadFromDatabase() {
        ...
        try {
            console.log('🔍 Loading comparison from database for manager...');
            const response = await fetch('/api/manager/comparison/load');
            
            if (response.ok) {
                const data = await response.json();
                console.log('📦 Database comparison data:', data);
                
                if (data.success) {
                    // Sync properties
                    if (data.properties && Array.isArray(data.properties)) {
                        this.comparisons = data.properties.map(id => String(id));
                        console.log('✅ Loaded properties from database:', this.comparisons);
                    }
                    
                    // Sync complexes
                    if (data.complexes && Array.isArray(data.complexes)) {
                        this.complexes = data.complexes.map(id => String(id));
                        console.log('✅ Loaded complexes from database:', this.complexes);
                    }
